...
import inngest
from inngest.experimental.ai.gemini import Adapter as GeminiAdapter, GenerationConfig, ThinkingConfig
...
def prepare_gemini_mixed_thinking_test() -> tuple[GeminiAdapter, dict[str, Any]]:
...
def handle_gemini_structured_response(response: dict[str, Any]) -> dict[str, Any]:
    return {
        "success": True,
        "response": extract_gemini_response(response),
        "raw_response": response,
...
def prepare_gemini_stop_sequences_test() -> tuple[GeminiAdapter, dict[str, Any]]:
...
def handle_gemini_stop_sequences_response(response: dict[str, Any]) -> dict[str, Any]:
    return {
        "success": True,
        "response": extract_gemini_response(response),
        "raw_response": response,
...
def prepare_gemini_conversion_test() -> tuple[GeminiAdapter, dict[str, Any]]:
    adapter = GeminiAdapter(auth_key=os.getenv("GEMINI_API_KEY") or "", model="gemini-2.0-flash-exp")
    body = {
        "messages": [
...
def handle_gemini_conversion_response(response: dict[str, Any]) -> dict[str, Any]:
    return {
        "success": True,
        "response": extract_gemini_response(response),
        "raw_response": response,
...
# Error Handling Test
def prepare_gemini_error_handling_test() -> tuple[GeminiAdapter, dict[str, Any]]:
    adapter = GeminiAdapter(auth_key="invalid-key-for-testing", model="gemini-2.0-flash-exp")
    body = {
...
def handle_gemini_error_handling_response(response: dict[str, Any]) -> dict[str, Any]:
